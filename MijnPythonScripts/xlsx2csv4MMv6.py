...
# v6 breaksdown function validateInputFile in subfunctions
...
def validateInputFile(fName):
    ...
    try:
        ...
        print ('\nInvalid fName signature:', fName, 'Expecting: Rreplication*YYYYMM.xlsx')
        raise
    else:
    ...
    # Sheet1 IR Exposure for zero% accounts with portfolio keys PR, RS, WS
...
def mainloop():
    ...
    exists(parserObj.infile, parserObj.sourcedir)
    hasValidSignature(parserObj.infile)
    noDuplicateProcessing(parserObj.infile, parserObj.targetdir)
    canRead(parserObj.infile, 'Sheet1')
    hasValidDimension(parserObj.infile, 'Sheet1')
    matchPortfolioKeys(parserObj.infile,'Sheet1',['PR','RS','WS'])
    noOtherNonBlankSheetsApartFrom(parserObj.infile, 'Sheet1')
